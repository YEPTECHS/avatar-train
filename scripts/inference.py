...
class VideoUNetInference:
    ...
    def preprocess_frame(self, frame: np.ndarray) -> torch.Tensor:
        ...
        source_img[:, :, ref_img.shape[2] // 2:, :] = 0
    ...
    def process_frame(
    ...
    ) -> torch.Tensor:  # Change return type to tensor
        ref_img, source_img, masked_source_img = self.preprocess_frame(frame)
        ...
        # Encode to latent space - modified for AutoencoderTiny
        ref_latent = self.vae_encoder.encode(ref_img.to(dtype=self.dtype)).latent_dist.sample() * self.vae_encoder.config.scaling_factor
        masked_latent = self.vae_encoder.encode(masked_source_img.to(dtype=self.dtype)).latent_dist.sample() * self.vae_encoder.config.scaling_factor
        ...
        if audio_feature is None:
            # add epsilon to avoid zero tensor
            audio_feature = torch.zeros(1, 10, 384, device=self.device, dtype=self.dtype) + 1e-6
            audio_feature = self.pe(audio_feature)
        
        # Run UNet forward pass
        output = self.unet(
            latent_input.to(dtype=self.dtype, device=self.device),
            timesteps,
            encoder_hidden_states=audio_feature
        ).sample
        
        # Scale latents
        output = output / self.vae_encoder.config.scaling_factor
        
        # Decode output
        decoded = self.vae_decoder.decode(output.to(torch.float32)).sample
        
        # Return processed tensor directly, without converting to PIL Image
    ...
    def process_image_sequence(
        ...
        try:
            ...
            for frame_idx in range(start_frame, end_frame):
                ...
                frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                real_tensor = torch.from_numpy(frame_rgb).permute(2, 0, 1)  # Convert to CHW format
                ...
                if audio_features_dir is None:
                    ...
                    audio_feature = audio_feature.reshape(-1, 384)  # 384 is whisper feature dimension
                ...
                # Process directly using tensor
                processed_tensor = self.process_frame(frame, audio_feature)
            ...
            # Create output paths for both videos
    ...
    def preprocess_img_tensor(self, image_tensor: torch.Tensor) -> torch.Tensor:
        """Uniform image preprocessing function"""
        N, C, H, W = image_tensor.shape
...
if __name__ == "__main__":
    model = VideoUNetInference(
        unet_path="/home/ubuntu/scratch4/kuizong/Avatar-Train/pretrained/finetune_4000",
    ...
    model.process_image_sequence(
        ...
        output_path="/home/ubuntu/scratch4/kuizong/Avatar-Train/output_finetune_4000.mp4",
